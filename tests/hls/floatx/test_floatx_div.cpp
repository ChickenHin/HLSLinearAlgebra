...
int main() {
    ...
    if (!std::isinf(hw_result) || hw_result < 0) {
        std::cerr << "ERROR: Division by zero test failed!" << std::endl;
        std::cerr << "  Expected: +inf" << std::endl;
        std::cerr << "  Actual:   " << hw_result << std::endl;
        error_count++;
    }
