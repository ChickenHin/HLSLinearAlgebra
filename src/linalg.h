...
struct mat
{
    ...
    template <typename __type, int __rows, int __cols>
    mat<type, _rows, _cols> operator*(mat<__type, __rows, __cols> c)
    {
        assert(_cols == __rows);
        assert(_rows == __cols);
...
template <typename type>
struct vec4 : public vec<type, 4>
{
    vec4() : vec<type, 4>()
    {
    }

    vec4(const vec<type, 4> &other) : vec<type, 4>(other)
    {
    }
...
template <typename type>
struct vec5 : public vec<type, 5>
{
    vec5() : vec<type, 5>()
    {
    }

    vec5(const vec<type, 5> &other) : vec<type, 5>(other)
    {
    }
...
struct _SE3
{
    ...
    _SE3(type qw, type qx, type qy, type qz, type tx, type ty, type tz)
    {
    ...
    _SE3(SO3<type> _rotation, vec3<type> _translation)
    {
        rot = _rotation;
        tra = _translation;
    }
    ...
    */
    ...
    _SE3<type> operator*(const _SE3<type> &rhs)
    {
        _SE3<type> result;
